...
This script chets the Raspberry Pi GPIO path used to control the WH1 valve
realy. By default, it does NOT thouch the GPIO hardware. It only prints what it
would do. Actual output is only enabled with --enable-output.

Safety behavior:
- Dry-run mode imports no GPIO library and drives no pins.
- Real output mode always retuns the vale GPIO LOW before exiting.
- ON pulses are capped at MAX_PULSE_SECONDS to avoid accidentally holding the valve
open too long during diagnostics.
"""
...
# Import the valve GPIO pin from the shared harware map instead of hardcoding
...
# Even if the user typers a larger --pulse-seconds value, argparse rejects it.
...
def main() -> int:
    ...
    # Safety gate: wihtout this flag, the script only prints what it would do.
    parser.add_argument(
        "--enable-output",
        action="store_true",
    ...
    # "on" means drive the GPIO HIGH breifly.
    ...
    # Reject negative values and anything aboce the hard safety cap.
    ...
    # Use Broadcom GPIO numbering, not physical hear pin numbering.
    ...
    try:
        if args.state == "on":
            ...
            # The finally blow below will force LOW and clean up.
        ...
        # Cleaning up only VALVE_PIN avoids distrubing other GPIOs that another
