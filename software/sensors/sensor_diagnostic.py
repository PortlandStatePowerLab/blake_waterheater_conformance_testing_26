...
def print_sensor_snapshot(
    ...
    print(
        ...
        "Converted values\n"
        "  Temperatures (degC)\n"
        f"    {'hot_temp_c':<18}: {snapshot.hot_temp_c:.3f} Â°C\n"
        f"    {'cold_temp_c':<18}: {snapshot.cold_temp_c:.3f} Â°C\n"
        f"    {'ambient_temp_c':<18}: {snapshot.ambient_temp_c:.3f} Â°C\n"
        "\n"
        "  Temperatures (degF)\n"
        f"    {'hot_temp_f':<18}: {snapshot.hot_temp_f:.3f} Â°F\n"
        f"    {'cold_temp_f':<18}: {snapshot.cold_temp_f:.3f} Â°F\n"
        f"    {'ambient_temp_f':<18}: {snapshot.ambient_temp_f:.3f} Â°F\n"
        "\n"
        "  Flow\n"
